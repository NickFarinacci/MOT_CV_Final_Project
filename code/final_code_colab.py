...
# Now set up video writers with all dimensions available
if args.output_file:
    output_filename = f'{args.output_file}.mp4'
    panel_filename = f'{args.output_file}_with_panel.mp4'
else:
    output_filename = f'output_{video_name}_{args.box_type}.mp4'
    panel_filename = f'output_{video_name}_{args.box_type}_with_panel.mp4'

# Keep videos in base directory
out_path = os.path.join(video_output_dir, output_filename)
panel_path = os.path.join(video_output_dir, panel_filename)

fourcc = cv2.VideoWriter_fourcc(*'mp4v')
out = cv2.VideoWriter(out_path, fourcc, fps, (width, height))
out_panel = cv2.VideoWriter(panel_path, fourcc, fps, 
                           (display_width + info_panel_width, display_height))
...
# Remove window creation since we're running headless
...
try:
    ...
    try:
        ...
        try:
            ...
            if "np.asfarray" in str(eval_error):
                # This error occurs after evaluation is complete, so we can ignore it
                logging.warning("Evaluation completed with minor error: %s", str(eval_error))
                logging.info("All evaluation results were saved successfully")
            else:
    ...
    logging.warning("- Video files may be incomplete")
