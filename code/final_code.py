"""
    final_code.py is the tesing code that uses evaluate.py to evaluate the model after it runs MOT on a input video.
    This code is used to test the model (which is easily interchangeable in the code) and evaluate its performance.
    ...
    In this code, we use cv2 to display the video in real-time with a sidebar info panel that displays:
    ...
    After the video is finished (which runs slow locally if using CPU), the code will then evaluate the performance of the model using evaluate.py.
    After evaluation, the code will then save the results into the output_files/<input filename>/evaluation_results folder.
    ...
    - Real-time visualization with interactive display
...
# Notice the difference here between this code and the Colab version.
...
try:
    ...
    # After the loop, run evaluation before cleanup
    try:
        # Import evaluation
